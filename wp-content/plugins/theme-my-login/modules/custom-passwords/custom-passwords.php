...
class Theme_My_Login_Custom_Passwords extends Theme_My_Login_Abstract {
	...
	public function password_fields() {
		...
		<p class="tml-user-pass1-wrap">
			...
			<br/>
			<input autocomplete="off" style="width:60%" name="pass1" id="pass1<?php $template->the_instance(); ?>" class="input" size="20" value="" type="password" />
		</p>
		<p class="tml-user-pass2-wrap">
			<label for="pass2<?php $template->the_instance(); ?>"><?php _e( 'Confirm Password', 'theme-my-login' ); ?></label>
			<br/>
			<input autocomplete="off" style="width:60%" name="pass2" id="pass2<?php $template->the_instance(); ?>" class="input" size="20" value="" type="password" />
		</p>
		<?php
	}
