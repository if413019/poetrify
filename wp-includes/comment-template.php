...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = true ) {
	...
	if ( false === $noreplytext ) $noreplytext = __( 'Komentar' );
	if ( false === $replytext ) $replytext = __( 'Komentar untuk %s' );
 ...
 *     @type string $cancel_reply_link    The translatable 'cancel reply' button label. Default 'Cancel'.
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'must_log_in'          => '<p class="must-log-in">' . sprintf(
		                              ...
		                              __( 'Silahkan <a href="%s">logged in</a> untuk menulis komentar.' ),
		                              wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) )
		                          ) . '</p>',
		/** This filter is documented in wp-includes/link-template.php */
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf(
		                              /* translators: 1: edit user link, 2: accessibility text, 3: user name, 4: logout URL */
		                              __( '<a href="%1$s" aria-label="%2$s">Anda login sebagai %3$s</a>. <a href="%4$s">Log out?</a>' ),
		                              get_edit_user_link(),
		                              /* translators: %s: user name */
		                              esc_attr( sprintf( __( 'Anda login sebagai %s. Edit your profile.' ), $user_identity ) ),
		                              $user_identity,
